...
class Options_Framework_Interface {
	...
	static function optionsframework_active_tabs() {
		$counter = 0;
		$options = & Options_Framework::_optionsframework_active_options();
		$menu = '';
		...
		foreach ( $options as $value ) {
			...
			case "colorradio":
				...
				foreach ($value['options'] as $key => $option) {
					...
					$selected = '';
					if ( $val != '' && ($val == $key) ) {
						$selected = ' of-colorradio-label-selected';
					}
					$output .= '<input class="of-input of-colorradio" type="radio" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="'. esc_attr( $key ) . '" '. checked( $val, $key, false) .' /><label style="background-color:'.$key.';" for="' . esc_attr( $id ) . '" class="of-colorradio-label' . $selected .'"></label>';
				}
				break;
	...
	static function optionsframework_active_fields() {
