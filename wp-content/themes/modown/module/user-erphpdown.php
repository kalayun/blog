	    ...
	    <div class="user-main">
	      ...
	      ?>
	          <div class="charge vip">
	                ...
	                <form id="charge-form2" action="" method="post">
		            	<h3><span>激活码升级</span></h3>
		              	<div class="item">
			                <input type="text" class="form-control input-recharge" id="erphpcard_num" name="erphpcard_num" required="" placeholder="激活码">
			            </div>
		                <div class="item">
			              	<input type="button" evt="user.vip.card.submit" value="立即升级" class="btn btn-card">
			            </div>
		            </form>
		        	<?php }?>
	          </div>
	      ...
	      <?php }elseif(isset($_GET['action']) && $_GET['action'] == 'ticket'){ 
	      		...
	      		if(isset($_POST['paytype']) && $_POST['paytype']){
					...
					{
						$url=constant("erphpdown")."payment/epay.php?ice_money=".$_POST['ice_money']."&type=alipay";
					}elseif(isset($_POST['paytype']) && $paytype==82)
					{
						$url=constant("erphpdown")."payment/epay.php?ice_money=".$_POST['ice_money']."&type=wxpay";
					}elseif(isset($_POST['paytype']) && $paytype==100)
					{
						$url=ERPHPDOWN_STRIPE_URL."/stripe.php?ice_money=".$_POST['ice_money'];
					}
					elseif(isset($_POST['paytype']) && $paytype==92)
					{
						$url=constant("erphpdown")."payment/payjs.php?ice_money=".$_POST['ice_money'];
					}
					elseif(isset($_POST['paytype']) && $paytype==91)
					{
						$url=constant("erphpdown")."payment/payjs.php?ice_money=".$_POST['ice_money']."&type=alipay";
					}
					elseif(isset($_POST['paytype']) && $paytype==102)
					{
						$url=constant("erphpdown")."payment/vpay.php?ice_money=".$_POST['ice_money'];
					}
					elseif(isset($_POST['paytype']) && $paytype==101)
					{
						$url=constant("erphpdown")."payment/vpay.php?ice_money=".$_POST['ice_money']."&type=2";
					}
					else{
	      	...
	      	?>
	          	<div class="charge">
	            	<div class="charge-header clearfix">
	                	...
	                	<div class="item item-tablet">
	                		<?php $userMoney=$wpdb->get_row("select * from ".$wpdb->iceinfo." where ice_user_id=".$current_user->ID);
	                		echo '<b>'.($userMoney?$userMoney->ice_get_money:'0.00').'</b> '.$moneyName;
	                		?>
	                		<p>站内消费</p>
	                	</div>
	                </div>
	            	...
	            	<form id="charge-form" action="" method="post">
		              	<div class="item" style="overflow: hidden;margin-bottom:0">
		              		...
		              		<?php }else{?>
			                <input type="number" min="0" step="0.01" class="form-control input-recharge" name="ice_money" id="ice_money" required="" placeholder="1 元 = <?php echo get_option('ice_proportion_alipay');?> <?php echo $moneyName;?>"><input type="submit" value="立即充值" class="btn btn-recharge" evt="user.charge.submit"><span class="rmb">元</span>
			            <?php }?>
			            </div>
			            <div class="item payment-radios">
			            			...
			            			foreach ($erphpdown_recharge_order_arr as $payment) {
			            				...
			            				switch ($payment) {
			            					...
			            					case 'f2fpay':
			            						...
			            						break;
			            					case 'paypal':
			            						echo '<input type="radio" id="paytype4" class="paytype"'.$checked.' name="paytype" value="4" /> <label for="paytype4" class="payment-label payment-paypal-label"><i class="icon icon-paypal"></i></label>';
			            						break;
			            					case 'paypy-wx':
			            					...
			            					case 'vpay-ali':
			            						...
			            						break;
			            					case 'stripe':
			            						echo '<input type="radio" id="paytype100" class="paytype" name="paytype" value="100"'.$checked.' /> <label for="paytype100" class="payment-label payment-stripe-label"><i class="icon icon-credit-card"></i></label>';
			            						break;
			            					default:
			            						break;
			            				}
			            	...
			            	?>
				                    ...
				                    <?php }?>
				                    <?php if(function_exists('plugin_check_stripe')){ if(plugin_check_stripe() && get_option('erphpdown_stripe_pk')){?>
					                <input type="radio" id="paytype100" class="paytype" name="paytype" value="100" checked/> <label for="paytype100" class="payment-label payment-stripe-label"><i class="icon icon-credit-card"></i></label>
					                <?php }}?>
					                ...
					                <input type="radio" id="paytype92" class="paytype" name="paytype" value="92" checked /><label for="paytype92" class="payment-label payment-wxpay-label"><i class="icon icon-wxpay-color"></i></label>
									<input type="radio" id="paytype91" class="paytype" name="paytype" value="91" /><label for="paytype91" class="payment-label payment-alipay-label"><i class="icon icon-alipay-color"></i></label>
					                <?php }?>
					                <?php if(get_option('erphpdown_codepay_appid')){?> 
					                <?php if(!get_option('erphpdown_codepay_alipay')){?><input type="radio" id="paytype71" class="paytype" name="paytype" value="71" checked /> <label for="paytype71" class="payment-label payment-alipay-label"><i class="icon icon-alipay-color"></i></label><?php }?>
					                <?php if(!get_option('erphpdown_codepay_wxpay')){?><input type="radio" id="paytype72" class="paytype" name="paytype" value="72" checked /> <label for="paytype72" class="payment-label payment-wxpay-label"><i class="icon icon-wxpay-color"></i></label><?php }?>
					                <?php if(!get_option('erphpdown_codepay_qqpay')){?><input type="radio" id="paytype73" class="paytype" name="paytype" value="73" /> <label for="paytype73" class="payment-label payment-qqpay-label"><i class="icon icon-qq"></i></label><?php }?>
					                <?php }?>
					                ...
					                <?php if(!get_option('erphpdown_vpay_wxpay')){?><input type="radio" id="paytype102" class="paytype" name="paytype" value="102" /> <label for="paytype102" class="payment-label payment-wxpay-label"><i class="icon icon-wxpay-color"></i></label><?php }?>
					                <?php }?>
			            	<?php }?>
		                </div>
		            </form>
				        	...
				        	echo '<div class="charge-gift">';
				          	...
				          	for($i=0; $i<$cnt;$i++){
				          		if($epd_game_price['buy'][$i]){
					          		echo '<div class="item">充值 <span>'.$epd_game_price['buy'][$i].'</span> 元，实际到账 <span>'.$epd_game_price['get'][$i]*get_option('ice_proportion_alipay').'</span> '.$moneyName.'</div>';
					          	}
				          	}
				          	echo '</div>';
				        }
