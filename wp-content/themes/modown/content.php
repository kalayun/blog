...
$lz = (_MBT('lazyload') && !_MBT('waterfall'))?1:0;
global $post_target;
?>
<div class="post grid<?php if($tj) echo ' grid-tj'; if($ts) echo ' grid-ts'; if($video) echo ' grid-vd'; if(_MBT('post_author')) echo ' grid-zz';?>"<?php if($ts) echo ' style="background-image:url('.MBThemes_thumbnail_full().')"';?> <?php if($video) echo 'data-video="'.$video.'"';?> data-id="<?php the_ID();?>">
    <div class="img">
        ...
        <?php if($video){?>
          ...
          <?php }else{?>
          <div class="grid-video"><video id="video-<?php the_ID();?>" autoplay="autoplay" preload="none" poster="<?php echo MBThemes_thumbnail();?>" src=""></video></div><span class="video-icon"><i class="icon icon-play"></i></span>
          <?php }?>
        <?php }?>
        </a>
    </div>
